...
void copyStruct(FieldList *list, FieldList *copylist) //list <-- copylist
{
    if(copylist==NULL)
    {
        //printf("copyStruct: copylist is NULL.\n");
        list=NULL;
        return;
    }
...
void copyParameterList(ParameterList *param, ParameterList *copyparam)
{
	if (copyparam == NULL)
	{
		//printf("copyParameterList: parameter is NULL.\n");
		param = NULL;
		return;
	}
...
TypeInfo* callfunc(TreeNode *p) // ID LP Args RP | ID LP RP
{
	...
	else
	{
		...
		else // ID LP Args RP
		{
			...
			else if(args(p->children[2], node->typeinfo->info.function->paralist) == 1)
				copyTypeInfo(type, node->typeinfo->info.function->functype);
			else
			{
				// error 9, arguments wrong
...
{
	// while (p->childrennum > 1) //Exp COMMA Args
	// {
	// 	if (param == NULL)
	// 		return 0;
	// 	TypeInfo *type = Exp(p->children[0]);
	// 	if (isEqualType(type, param->typeinfo) == 0)
	// 		return 0;
	// 	p = p->children[2];
	// 	param = param->next;
	// }
	...
	for(; q->childrennum>1; q=q->children[2], pa=pa->next)
	{
		...
		// printf("diaoyongde type\n");
		// printTypeInfo(type);
		// printf("hanshude type\n");
		// printTypeInfo(pa->typeinfo);
